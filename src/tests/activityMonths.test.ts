...
const extractMonths = (customFields: Record<string, unknown>): string[] =>
  extractActivityMonths(customFields, "bolsaOperations")

/** Builds the customFields shape a loader persists. */
function withOperations(operations: unknown): Record<string, unknown> {
  return { bolsaOperations: JSON.stringify(operations) }
}

describe("extractActivityMonths", () => {
  it("returns the month of a single operation", () => {
    expect(extractMonths(withOperations([{ date: "01/05/2026" }]))).toEqual(["2026-05"])
  })
  ...
  it.each([
    ...
    expect(extractMonths(customFields as Record<string, unknown>)).toEqual([])
  })

  it("accepts a single-digit day, which the loaders could emit", () => {
    expect(extractMonths(withOperations([{ date: "1/05/2026" }]))).toEqual(["2026-05"])
  })
})
