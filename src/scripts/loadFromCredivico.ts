...
import { CredivicoLoader } from "@infrastructure/loaders/CredivicoLoader.js"
import { LoaderService } from "@application/LoaderService.js"

/**
 * Driving adapter (CLI) for the "Empresas Credivico" source.
 *
 * Wires the CredivicoLoader, NosisEnricher, and Neo4jRepository together
 * and delegates orchestration to the LoaderService.
 *
 * Usage:
 *   pnpm tsx src/scripts/loadFromCredivico.ts <inputPath> [startRow] [count]
 *
 * Example:
 *   pnpm tsx src/scripts/loadFromCredivico.ts ./sources/empresas-credivico.xlsx 1 100
 */
async function main(): Promise<void> {
  const inputPath = process.argv[2]
  if (!inputPath) {
    throw new Error("Usage: loadFromCredivico <inputPath> [startRow] [count]")
  }
  ...
  const loader = new CredivicoLoader(inputPath)
